...
Within the bachelor's degree in computer science, the discipline of compilers has considerable difficulty in their learning, so it becomes important tools that help in understanding the content of the discipline. The development of the D + interpreter has made it possible to approach the teaching of compiler material in a more illustrative way, by illustrating to students more clearly the compilation process. The implementation of the framework was made using QT Creator software, and creation of a modified D + compiler to become an Interpreter. To obtain learning data using the D + Interpreter was made available to the students and asked to answer a questionnaire. From these questionnaires we can conclude that the interpreter helped the students to better understand the discipline by showing more compilation of the code.
...
\begin{figure}[htb]
    \legenda[fig:figura]{Autômoto de validação de caractere}
